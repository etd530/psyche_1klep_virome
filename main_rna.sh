...
if [ -s ../3_viral_contig_identification/${reads_pre}.Trinity.vs2.out/${reads_pre}.Trinity.fa ]; then
    ...
    awk -F"\t" '{if (($8 == "High-quality" || $9 == "High-quality") && $14 == "" && $6 >= 1 && $3 == "No") print $0}' ${reads_pre}.Trinity.checkv/quality_summary.tsv > ${reads_pre}.checkv_trinity.high_quality.viruses.txt
    awk -F"\t" '{if (($8 == "High-quality" || $9 == "High-quality") && $14 == "" && $6 >= 1 && $3 == "Yes") print $0}' ${reads_pre}.Trinity.checkv/quality_summary.tsv > ${reads_pre}.checkv_trinity.high_quality.proviruses.txt
    # grep High-quality ${reads_pre}.Trinity.checkv/quality_summary.tsv | cut -f1,3 | grep 'No' | cut -f1 > ${reads_pre}.checkv_trinity.high_quality.viruses.txt || true       # adding '|| true' prevents empty grep from causing an error
    # grep High-quality ${reads_pre}.Trinity.checkv/quality_summary.tsv | cut -f1,3 | grep 'Yes' | cut -f1 > ${reads_pre}.checkv_trinity.high_quality.proviruses.txt || true   # we don't want empty grep as an error since some cases will have only proviruses or only free viruses
...
# if [ -s ../4_checkv/${reads_pre}.checkv_trinity.high_quality.viruses.fa ]; then
#     /usr/bin/time -o ${reads_pre}.time -v bash ./mummer.sh ../4_checkv/${reads_pre}.checkv_trinity.high_quality.viruses.fa ${ucomp_genome}
# else
#     echo -e "WARNING: No high-quality free viruses to align for ${reads_pre}"
# fi

# # Run for proviruses if any exist
# if [ -s ../4_checkv/${reads_pre}.checkv_trinity.high_quality.proviruses.fa ]; then
#     /usr/bin/time -o ${reads_pre}.time -v bash ./mummer.sh ../4_checkv/${reads_pre}.checkv_trinity.high_quality.proviruses.fa ${ucomp_genome}
# else
#     echo -e "WARNING: No high-quality free viruses to align for ${reads_pre}"
# fi
