...
if [ -e ${sample}.DONE ]; then
	...
	minimap2 -ax splice:sr -t $threads -R "@RG\tID:${sample}\tSM:${sample}" $ref_genome $file1 | samtools sort -@${threads} -o ${sample}.vs.${genome_prefix}.bam && samtools index ${sample}.vs.${genome_prefix}.bam && \
